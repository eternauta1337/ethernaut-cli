  ...
  describe('when asked to convert SNX to bytes', function () {
    describe('without prompting', function () {
      before('call interpret', async function () {
        await terminal.run(
          'npx nyc hardhat ai interpret "what is SNX in bytes?" --no-confirm --new-thread --model \'gpt-3.5-turbo\'',
          60000,
        )
      })
    ...
    describe('with prompting', function () {
      before('run interpret', async function () {
        await terminal.run(
          'npx nyc hardhat ai interpret "what is SNX in bytes?" --new-thread --model \'gpt-3.5-turbo\'',
          30000,
        )
      })
